...
- сбор и хранение фотографий в базе данных, формирование датасета для тренировки нейросетей;
...
1. Определяется участок местности, в котором будет осуществляться поиск пропавшего человека.
2. Оператор запускает БПЛА в соответствии с подготовленным маршрутом.
3. БПЛА осуществляет фото или видео съемку и передает информацию на терминал оператора (ноутбук или смартфон) с установленным ПО RescuerLaApp.
4. С терминала полученные снимки отправляются на сервер, где происходит обработка снимков с помощью нейросетей. В случае обнаружения человека координаты снимка передаются на терминал и выводятся оператору.
